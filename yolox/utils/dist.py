...
def _serialize_to_tensor(data, group):
    ...
    device = paddle.device("cpu" if backend == "gloo" else "cuda")
...
def _pad_to_largest_tensor(tensor, group):
    ...
    local_size = paddle.to_tensor([tensor.numel()], dtype="int64", place=tensor.device)
    size_list = [
    ...
    if local_size != max_size:
        padding = paddle.zeros(
            [max_size - local_size,], dtype="uint8"
        )
...
def all_gather(data, group=None):
    ...
    tensor_list = [
        paddle.empty([max_size,], dtype='uint8')
        for _ in size_list
    ]
